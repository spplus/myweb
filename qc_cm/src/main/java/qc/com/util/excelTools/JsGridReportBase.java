...
public class JsGridReportBase {
	...
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData) throws Exception {
		...
		for (int c = 0; c < headerMetaData.getColumns().size(); c++) {
			...
			//if(sheet.getColumnWidth(c)<t.length()*256*3){
				...
				//sheet.setColumnWidth(c, t.length()*256*7);
			//}
		}
	 ...
	 * @param tableData  表格数据
	 */
	public void exportToExcel(String filename,String title, String creator, TableData tableData,List<Region> riList)throws Exception {
		/**创建新的Excel 工作簿**/
		...
		for(Region ri:riList){
			sheet.addMergedRegion(ri); 
		}
        ...
        if( isMSIE ){
             finalFileName = java.net.URLEncoder.encode(sFileName,"UTF8");
        }else{
        	if(agent != null && agent.indexOf("11.0") != -1){
        		finalFileName = java.net.URLEncoder.encode(sFileName,"UTF8");
        	}else{
                finalFileName = new String(sFileName.getBytes("UTF-8"), "ISO-8859-1");
        	}
        }
	...
	private void createCell(HSSFRow row, TableColumn tc,List<TableDataCell> data, int i, int index,HashMap<String, HSSFCellStyle> styles) {
		...
		case TableColumn.COLUMN_TYPE_FLOAT_2:
			Double dl = dc.getDoubleValue();
			if(dl==0d){
				cell.setCellValue(0);
				style = styles.get("INT");
				if (row.getRowNum() % 2 != 0){
					style = styles.get("INT_C");
				}
			}else{
