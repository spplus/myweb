 ...
 * 
 * @autho：Liaoluping
 * @version: 1.0, $Date：2011-10-26$
 */
public class BeanSort {

    private static Logger logger = Logger.getLogger("CollectionUtils");
    /**
     ...
     * @param isDesc 是否降序排列
     * @param properties要排序的属性。前面的值优先级高。
     * 
     */
    ...
    /**
     * 
     * 利用反射调用Bean的getter方法，再比较值。 如果getter方法调用失败，排序方法将失效。
     * 
     * 如果getter方法返回值为Null，则认为此Bean小于另一Bean。 当两个返回值同时为null时，认为第一个Bean更小。
     * 其余情况下由compareTo方法决定比较结果。
     * 
     * @param methodName
     * @param isDesc
     * @return
