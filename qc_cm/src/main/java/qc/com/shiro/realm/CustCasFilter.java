 ...
 * @since 1.2
 */
public class CustCasFilter extends AuthenticatingFilter {
    ...
    protected boolean onLoginFailure(AuthenticationToken token, AuthenticationException ae, ServletRequest request,
        ...
        if (subject.isAuthenticated() || subject.isRemembered()) {
            try {
                ...
                HpLogger.error("Cannot redirect to the default success url", e);
            }
        } else {
            try {
                WebUtils.issueRedirect(request, response, failureUrl);
            } catch (IOException e) {
            	HpLogger.error("Cannot redirect to failure url : "+failureUrl, e);
            }
        }
        return false;
    }
